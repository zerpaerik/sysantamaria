			...
			<div class="box-content">
				...
				<form class="form-horizontal" role="form" method="post" action="consulta/edit">
					...
					<label class="col-sm-1 control-label">Pacientes</label>
						...
						<div class="col-sm-3">
							<select id="el3" name="evaluaciones">
								@foreach($evaluaciones as $eval)
									<option value="{{$eval->id}}">
										{{$eval->nombre}} -Precio:{{$eval->precio}} 
									</option>
								@endforeach
							</select>
						</div>
						...
						<a href="consulta" style="margin-left:15px; margin-top: 20px;" class="col-sm-2 btn btn-danger">Volver</a>
					</div>			
				</form>	
			</div>
		</div>
